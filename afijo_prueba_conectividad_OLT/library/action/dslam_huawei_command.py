...
DOCUMENTATION = '''
---
module: dslam_huawei_command

short_description: Huawei DSLAM devices command module
...
HUAWEI_ERROR_RC = ["Unknown command, the error locates at", 
...
class ActionModule(ActionBase):
    ...
    def _initialize(self, task_vars):
        ...
        if self._enable:
            ...
            init_cmd = ['enable', 'undo event output all', 'undo alarm output all', {'scroll' : [{'{ <cr>\|number<U><10,512>|<cr>\|scrollnum<U><10,512> }:' : ' '}]}]
            self._display.vvv('SETTING ENABLE MODE', host=self._play_context.remote_addr)
        else:
            init_cmd = [{'scroll' : [{'{ <cr>\|number<U><10,512>|<cr>\|scrollnum<U><10,512> }:' : ' '}]}]
        ...
        # comando de salida
        self._connection._exit_cmd = "quit"
        self._connection._exit_confirm = {'\(y\/n\)\[n\]:' : 'y'}
