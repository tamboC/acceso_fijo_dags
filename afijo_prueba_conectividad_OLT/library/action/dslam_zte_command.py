...
DOCUMENTATION = '''
---
module: dslam_zte_command

short_description: ZTE DSLAM devices command module
...
options:
  ...
  enable:
      type: bool
      description: 
        - True when enable mode is required. Otherwise enable mode is disabled
      vars:
          - name: enable
'''
...
ZTE_9210_ERROR_RC = ["Error: Bad command"]
ZTE_9800_ERROR_RC = ["Invalid input detected at"]
CMD_ERROR_RC = ZTE_9210_ERROR_RC + ZTE_9800_ERROR_RC
...
class ActionModule(ActionBase):
    ...
    def _get_additional_params(self):
        ...
        self._init_cmd = self._task.args.get('init_cmd')
        if self._init_cmd and not isinstance(self._init_cmd, list):
            raise AnsibleError("Parameter init_cmd must be a list type.")
        ...
        self._enable = self._task.args.get('enable')
        if self._enable and not isinstance(self._enable, bool):
            raise AnsibleError('Parameter enable must be a boolean.')
        ...
        self._init_delay = init_delay if init_delay and init_delay > 0 else 2
    ...
    def _initialize(self, task_vars):
        ...
        def _get_connection_arg(pc, arg, default_arg=None):
            # TEMPORAL: Borrar una vez que se reciba el enable_pass por inventario
            if hasattr(pc, 'connection_args') and arg in pc.connection_args:
                return pc.connection_args[arg]
            else:
                return default_arg
        ...
        if self._enable:
            enable_pass = _get_connection_arg(self._play_context, 'enable_pass', 'anacrusa79')
            init_cmd = [{'enable' : [{'Password:' : enable_pass }]}]
            self._display.vvv('SETTING ENABLE MODE', host=self._play_context.remote_addr)
        else:
            init_cmd = []

        # parametros de paginado
        self._connection._pagination_prompt = ["--More--", "Press any key to continue \(Q to quit\)"]
        self._connection._pagination_delay = _get_connection_arg(self._play_context, 'network_delay', 1)

        # conexion
        self._connection._connect()
        # delay de inicializacion
        # los ZTE necesitan un init_delay luego de conectar, por defecto 2 segundos
        if self._init_delay:
        ...
        self._display.vvv('INIT_CMD: BEGIN EXECUTION', host=self._play_context.remote_addr)
        for cmd in init_cmd:
            sleep(self._intercmd_delay)
            cmd = self._parse_command(cmd)
            stdout = self._low_level_execute_command(cmd, executable=False)["stdout"]
            if any(cmd_error in stdout for cmd_error in CMD_ERROR_RC):
                raise AnsibleError('Error executing \'{cmd}\': \'{stdout}\''.format(cmd=cmd, stdout=stdout))
        self._display.vvv('INIT_CMD: EXECUTED SUCCESSFULLY', host=self._play_context.remote_addr)
