...
short_description: generic telnet command module
...
options:
  ...
  init_cmd:
      description: 
        - List of initial commands to be executed before command (executed only once).
        - Interactive communication when a list of prompts and their respective responses is provided. See example below.
      vars:
           - name: init_cmd
      suboptions:
          ...
          description: 
           - list of dict where each key is a regular expression expected and their respective value is the response to send.
           - if the first key is 'cmd', its value will be executed instead of 'init_cmd'. This is useful when trying to template cmd because ansible does not template dict keys.
...
TELNET_ERROR_RC = ["Bad command", "Unknown command", "Fatal error"]
CMD_ERROR_RC = TELNET_ERROR_RC
